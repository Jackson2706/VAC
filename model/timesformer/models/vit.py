...
from model.timesformer.models.vit_utils import IMAGENET_DEFAULT_MEAN, IMAGENET_DEFAULT_STD
from model.timesformer.models.helpers import load_pretrained
from model.timesformer.models.vit_utils import DropPath, to_2tuple, trunc_normal_
# from timesformer.models.moose import MOOSE
...
raft_args = parser.parse_args(['--model', '/home/jackson-devworks/Desktop/VAC/weights/raft-things.pth'])

@MODEL_REGISTRY.register()
class MOOSE(nn.Module):
    def __init__(self, cfg, raft_args = raft_args, **kwargs):
        ...
        with torch.no_grad():
            self.moose_encoder = MOOSE_Encoder(raft_args)
        self.patch_embed = MotionPatchEmbed(img_size=28, patch_size=2, in_chans=2, embed_dim=768) # Flow patches embedding
        num_classes = 400
    ...
    def forward(self, x):
        with torch.no_grad():
            visual_embeddings, flow_low = self.moose_encoder(x)
        # visual_embeddings, flow_low = x[0], x[1]
        # print(visual_embeddings.shape, flow_low.shape)
        # assert False
        b = flow_low.shape[0]
        t = flow_low.shape[1]
        flow_low = rearrange(flow_low, 'b t a c w h -> (b t a) c w h')
        # print(flow_low.shape)
        video_embeddings = visual_embeddings
        motion_embeddings = self.patch_embed(flow_low)
